...
BINTRAY_VERSION_PATH=${BINTRAY_OWNER}/${BINTRAY_REPO}/${PCK_NAME}/${PCK_VERSION}
...
function deploy_updatesite() {
  ...
  if [ ! -d $f ]; then
    ...
    then
      ...
      curl -X PUT -T $f -u ${BINTRAY_USER}:${BINTRAY_API_KEY} ${API}/content/${BINTRAY_OWNER}/${BINTRAY_REPO}/$f;publish=0
    else 
  ...
  curl -X POST -u ${BINTRAY_USER}:${BINTRAY_API_KEY} ${API}/content/${BINTRAY_VERSION_PATH}/publish -d "{ \"discard\": \"false\" }"
}
